...
\begin{frame}{模型计算流程}
  ...
  \[
    D_{t-1} = \{\omega_{t-1,1},\cdots,\omega_{t-1,m}\},\qquad
...
\begin{frame}{AdaLab：AdaBoost 实验园地}
\textbf{AdaLab} 是一个面向研究的 AdaBoost 实验园地，通过端到端的实验框架支持 AdaBoost 算法的过拟合行为、泛化能力与鲁棒性特征的系统性研究。  
项目提供了 Python 实验框架与 CLI 工具，支持通过 JSON 配置文件驱动实验流程，实现零代码运行实验适用于算法分析、实验复现与对比研究。

\begin{itemize}
    \item 端到端实验框架从训练、评估到可视化，所有步骤都可以通过 JSON 配置文件驱动。
    \item AdaBoost 核心机制可观测支持样本权重、噪声样本与干净样本权重、弱分类器 alpha 等的精细监控。
    \item 鲁棒性研究的数据处理模块包括特征提取（HOG、Hu 不变矩）和多种图像扰动方式（噪声、破坏、变形等）。
    \item CLI 驱动，零代码运行提供统一的 adalab 命令行工具，适合批量实验管理与脚本化操作。
    \item 可选配套可视化模块独立的 adalab\_viz 包，用于结构化可视化实验结果。
\end{itemize}

\end{frame}


\begin{frame}{AdaLab 使用方法与常见运行模式}
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{./assets/adalab_help.png}
    \caption{adalab包的CLI使用}
  \end{figure}
...
\begin{frame}{AdaBoost参数选择}
  \begin{table}[]
    ...
    \textbf{max\_features} & 0.3                    \\ \hline
    \textbf{criterion}     & entropy                \\ \hline
    \textbf{random\_state} & 42                     \\ \hline
    \textbf{n\_estimators} & 500                   \\ \hline
...
\begin{frame}{原始图片}
  \begin{table}[]
    ...
    \textbf{MNIST}        & 0.9184            & 0.9196                  & 0.9180                  & 0.9182                  \\ \hline
    \textbf{课程数据集}   & 0.6               & 0.4                  & 0.6                     & 0.4667                  \\ \hline
    \end{tabular}
...
\begin{frame}{Hu不变矩}
  \begin{table}[]
    ...
    \hline
    \textbf{数据集}       & \textbf{准确率}    & \textbf{精度 (宏平均)}   & \textbf{召回率 (宏平均)} & \textbf{F1值 (宏平均)} \\ \hline
    \textbf{MNIST}        & 0.5208            & 0.5017                  & 0.5113                  & 0.5038                  \\ \hline
    \textbf{课程数据集}   & 0.4               & 0.2333                    & 0.4                     & 0.2833                  \\ \hline
    \end{tabular}
    \caption{模型在MNIST和课程数据集上的测试结果}
  \end{table}
\end{frame}
...
\begin{frame}{理想模型数据展示(HOG)}
  \begin{table}[]
    ...
    \textbf{pixels\_per\_cell} & [4, 4]             \\ \hline
  ...
  \begin{table}[]
  	...
  	\begin{tabular}{|c|c|c|c|c|}
  		\hline
  		\textbf{数据集}       & \textbf{准确率}    & \textbf{精度 (宏平均)}   & \textbf{召回率 (宏平均)} & \textbf{F1值 (宏平均)} \\ \hline
  		\textbf{MNIST}        & 0.958            & 0.9581                  & 0.9578                  & 0.9579                  \\ \hline
  		\textbf{课程数据集}   & 0.6               & 0.525                  & 0.6                     & 0.54                  \\ \hline
  	\end{tabular}
  	\caption{模型在MNIST和课程数据集上的测试结果}
  \end{table}
\end{frame}
...
\begin{frame}{泛化能力:MNIST数据量下的结论}
\begin{table}[htbp]
\centering
\begin{tabular}{|c|c|c|}
\hline
\textbf{测试集} & \textbf{准确率} & \textbf{平均Recall} \\ \hline
Contrast & 0.958 & 0.960 \\ \hline
Rotate & 0.958 & 0.960 \\ \hline
Brightness & 0.958 & 0.960 \\ \hline
Multiple Disturbances & 0.916 & 0.922 \\ \hline
\end{tabular}
\end{table}

\textbf{备注：} 
对于多个扰动测试集，尽管准确率有所下降（0.916），但大部分类别的Recall值较高，尤其是数字"0"、"6"和"8"的Recall均维持在较高水平。少数类别（如"1"和"7"）的Recall较低，表明其泛化能力受到了影响。
\end{frame}

\begin{frame}{泛化能力:特征提取有效性}
  不同特征提取方式的特征空间分布：
  \begin{figure}
  	\centering
  	\begin{tabular}{ccc}
...
\begin{frame}{鲁棒性：模型对噪声的适应能力}
  \begin{figure}
  	\centering
  	\begin{tabular}{ccc}
  		\includegraphics[width=0.85\textwidth]{./assets/noise6_hog_depth2_v2.png} 
  	\end{tabular}
  \end{figure}
